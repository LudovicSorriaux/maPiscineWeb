...
// Signaler au bootstrap que les handlers de la page graphs sont prêts
try {
	window.piscineGraphsLoaded = true;
	if (window.debug) console.info('[piscineGraphs] flag piscineGraphsLoaded = true');
} catch (e) {
	try { console.warn('[piscineGraphs] unable to set piscineGraphsLoaded flag', e); } catch(_){}
}
...
function applyChartHeights() {
    const headerH = $('#graphHeader').outerHeight() || 42;
    const periodH = $('#periodControlBox').outerHeight() || 0;
    const navH = $('#navZone').outerHeight() || 65;
	console.debug("[applyChartHeights] headerH:", headerH, "periodH:", periodH, "navH:", navH, "innerHeight:", window.innerHeight);

    // padding géré entièrement par CSS : #pagePiscineGraphs .ui-content { padding: 0 10px !important; }
    $('#graphContent').css({ 'overflow': 'hidden' });

    // dashboard-card : hauteur totale moins header et barre de période
    const cardH = Math.max(window.innerHeight - headerH - periodH - 5, 100);
    $('.dashboard-card').css({ 'height': cardH + 'px', 'width': '100%' });

    // charts-wrapper : hauteur de la card moins le navigateur
    const availableDataH = Math.max(cardH - navH - 5, 50);
    ...
    // Visibilité et largeur des zones selon le layout
	if (layout.orientation === 'row') {
		const colPercent = Math.floor(100 / layout.count);
		for (let i = 1; i <= 3; i++) {
			const $zone = $(`#graph-zone-${i}`);
			if (i === 1 || i <= layout.count) {
				$zone.show().css({ 'display': 'flex', 'width': colPercent + '%', 'min-width': 0 });
			} else {
				$zone.hide().css({ 'width': '0%', 'min-width': 0 });
			}
		}
	} else {
		for (let i = 1; i <= 3; i++) {
			const $zone = $(`#graph-zone-${i}`);
			if (i === 1 || i <= layout.count) {
				$zone.show().css({ 'display': 'flex', 'width': '100%', 'min-width': 0 });
			} else {
				$zone.hide();
			}
		}
			...
			try {
				...
				try {
					...
					if ((dayRows.length || 0) > 0 && (dayRows.length < expectedPerDay)) {
						...
						const fetched = await fetchDataChunked(dayjs(genStart), dayjs(genEnd));
...
function initCharts() {
	...
	const layout = determineLayout();
    // 3. Calculer les hauteurs d'abord pour que Dygraph connaisse sa taille
	applyChartHeights();

	// Construire les selecteurs d'axes dynamiquement pour chaque zone (1..3)
	for (let z = 1; z <= 3; z++) createAxesSelectorForZone(z);

    // 4. Créer les instances de graphiques (ils vont lire getGraphOptions)
    // 4.1. On coupe la synchro avant de toucher au DOM
    if (syncHandler) { 
        try { syncHandler.detach(); } catch(e) {} 
        syncHandler = null; 
    }
	// 4.2 Création des Dygraphs affichés par zone (1..3) + navigator
	...
	// 5. Navigator
	if (!window.graphInstances || !Array.isArray(window.graphInstances)) return;
	...
	// 6. ResizeObserver (votre code est très bien ici) — ajouter des logs de diagnostic
...
*/
	...
	$(document).delegate("#pagePiscineGraphs","pagebeforecreate", function() {
		...
		window._initDataPromise = new Promise(function(resolve) {
...
// Signal readiness to other scripts (e.g. `piscineScripts.js`).
// Placer ce dispatch ici garantit que les handlers pagebeforecreate/pageshow
// ont été enregistrés dans ce fichier avant de prévenir l'extérieur.
try {
	try { window.piscineGraphsLoaded = true; } catch(e){}
	try { window.piscineGraphsIsReady = true; } catch(e){}
	document.dispatchEvent(new Event('piscineGraphsLoaded'));
	console.debug && console.debug('[piscineGraphs] dispatched piscineGraphsLoaded');
} catch (e) {
	try { console.warn('[piscineGraphs] readiness dispatch failed', e); } catch(_){}
}
...
// --- page PiscineGraphs Phase 2 : pageshow ---
	$(document).on("pageshow","#pagePiscineGraphs",function(){
		...
		setTimeout(async function() {
			try {
				// Attendre que initData() soit terminé avant d'initialiser les graphiques
				if (window._initDataPromise) {
					await window._initDataPromise;
				}
				initCharts();
			} finally {
				$.mobile.loading('hide');
		...
		// Bloquer le swipe-to-navigate JQM quand on manipule le navigateur (range slider).
		// touchstart doit aussi être bloqué : JQM enregistre la position initiale sur touchstart,
		// ce qui suffit à déclencher swipeleft/swiperight même si touchmove est stoppé.
		$('#navZone').on('touchstart.graphswipe touchmove.graphswipe', function(e) {
			e.stopPropagation();
		});
	});

	$(document).on("pagehide","#pagePiscineGraphs",function(){
		$('#navZone').off('.graphswipe');
	});
